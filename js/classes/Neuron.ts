...
type ActivationType = 'sigmoid' | 'linear' | 'relu' | 'step' | 'stepAbs'

class Neuron {
  activationType: ActivationType
  connectionTo: { neuron: Neuron, weight: number }[] = []
  connectionFrom: { neuron: Neuron, weight: number }[] = []
  value: number

  constructor(activationType: ActivationType) {
    this.activationType = activationType
  }

  linkTo(neuron: Neuron, weight: number) {
...
const ActivationTypes = {
  sigmoid: (x) => 2 / (1 + Math.E ** (-x)) - 1,
  tanh: (x) => Math.tanh(x),
  tanh_prime: (x) => 1 / (Math.cosh(x) ** 2),
